...
interface ConfigBlock
  extends Record<
    ...
    | ConfigBlock
    | ConfigBlock[]
    | (string | number | number[] | ConfigBlock)[]
  > {}
  ...
  function parseOne(parent: ConfigBlock) {
    ...
    switch (typeCode) {
      case TypeCode.CONTAINER:
        ...
        if (name in parent) {
          const prevVal = parent[name]!;
          if (Array.isArray(prevVal)) {
            parent[name] = [...prevVal, newContainer];
          } else {
            parent[name] = [prevVal, newContainer];
          }
        } else {
...
console.dir(parse(contents), { depth: null });
